...
const ManageHostsPage = ({
  ...
  // ========= states
  const [showNoEnrollSecretBanner, setShowNoEnrollSecretBanner] =
    useState(true);
  ...
  const onDeleteHostSubmit = async () => {
    ...
    try {
      ...
      const successMessage = `${
        selectedHostIds.length === 1 ? "Device" : "Devices"
  ...
  const renderHostCount = useCallback(() => {
    return (
      <>
        <TableCount name="devices" count={totalFilteredHostsCount} />
        {!!totalFilteredHostsCount && (
          <Button
  ...
  const renderTable = () => {
    ...
    if (maybeEmptyHosts) {
      const emptyState = () => {
        const emptyHosts: IEmptyTableProps = {
          ...
          info: "Expecting to see devices? Try again soon as the system catches up.",
        };
        if (includesFilterQueryParam) {
          delete emptyHosts.graphicName;
          emptyHosts.header = "No devices match the current criteria";
          emptyHosts.info =
            "Expecting to see new devices? Try again soon as the system catches up.";
    ...
    const emptyState = () => {
      const emptyHosts: IEmptyTableProps = {
        header: "No devices match the current criteria",
        info: "Expecting to see new devices? Try again soon as the system catches up.",
      };
      if (isLastPage) {
        emptyHosts.header = "No more devices to display";
        emptyHosts.info =
          "Expecting to see more devices? Try again soon as the system catches up.";
      }
